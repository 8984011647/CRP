% function [Number_Nodes_Alive, first_dead, Throughput, CH_Energy, Std_Energy, Average_Cluster_Head_Energy, Average_Standard_Deviation_Energy, Average_Fraction, Fraction_CH] = run_CRP (m, a,Num_Nodes, Popt)
m = 0.2;
a = 3;
Num_Nodes = 100;
Popt = 0.1;
...
if (To_Plot ==1)
   figure(1);
   hXLabel = xlabel('X Coordinate \rightarrow'                     );
   hYLabel = ylabel('Y Coordinate \rightarrow'                      );
   hold on;
end
      for i=1:1:n
          temp_rnd0=i;
  ...
  if ((S(i).epoch < Epoch_H) && (S(i).ENERGY == 0))
    for i=1:1:n
        S(i).G=1;
        S(i).cl=0;
    end
...
%             PACKETS_TO_BS(r+1)=packets_TO_BS;
            ...
            S(i).type='C';
            Cluster_Initial (countCHs, 1) = S(i).xd;
            Cluster_Initial (countCHs, 2) = S(i).yd;
             C(cluster).xd=S(i).xd;
             C(cluster).yd=S(i).yd;
...
for i=1:1:n
   if ( S(i).type=='N' && S(i).E>0 )
     if(cluster-1>=1)
       ...
       Cluster_Size (min_dis_cluster) = Cluster_Size (min_dis_cluster) + 1;
       Members (min_dis_cluster, Cluster_Size (min_dis_cluster)) = i;
...
Number_Advanced_Nodes = 0;
% Reform Clusters
for c = 1 : cluster-1
    Max_Energy = S(Members(c,1)).E;
    Max_Energy_Index = Members(c,1);
    Cluster_Head = Max_Energy_Index;
    for j = 2 : Cluster_Size (c)
        Member_Id = Members(c, j);
        if (Max_Energy < S(Member_Id).E)
            S (Max_Energy_Index).type = 'N';
            Max_Energy_Index = Member_Id;
            Max_Energy = S(Member_Id).E;            
        end
    end
    
    Cluster_Head (c) = Max_Energy_Index;
    S(Max_Energy_Index).type = 'C';
    if (S(Max_Energy_Index).ENERGY == 1)
        Number_Advanced_Nodes = Number_Advanced_Nodes + 1;
    end
    Cluster_Head_Energy = Cluster_Head_Energy + S(Max_Energy_Index).E;
    Cluster (c, 1) = S(Max_Energy_Index).xd;
    Cluster (c, 2) = S(Max_Energy_Index).yd;
%     if (To_Plot == 1)
%       plot(S(Max_Energy_Index).xd, S(Max_Energy_Index).yd,'k*');
%     end
    
    packets_TO_BS=packets_TO_BS+1;
    
    for j = 1 : Cluster_Size (c)
        Member_Id = Members(c, j);
        if (S(Member_Id).type == 'C')
            distance=sqrt( (S(Member_Id).xd-(S(n+1).xd) )^2 + (S(Member_Id).yd-(S(n+1).yd) )^2 );
            if (distance>do)
                S(Member_Id).E=S(Member_Id).E- ( (ETX+EDA)*(4000) + Emp*4000*( distance*distance*distance*distance )); 
            end
            if (distance<=do)
                S(Member_Id).E=S(Member_Id).E- ( (ETX+EDA)*(4000)  + Efs*4000*( distance * distance )); 
            end
        else
            Distance_To_CH = sqrt( (S(Member_Id).xd-(S(Cluster_Head (c)).xd) )^2 + (S(Member_Id).yd-(S(Cluster_Head (c)).yd) )^2 );
            if (Distance_To_CH>do)
                S(Member_Id).E=S(Member_Id).E- ( ETX*(4000) + Emp*4000*( Distance_To_CH ^4)); 
            else
                S(Member_Id).E=S(Member_Id).E- ( ETX*(4000) + Efs*4000*( Distance_To_CH ^2));
            end
            S(Cluster_Head (c)).E = S(Cluster_Head (c)).E- ( (ERX + EDA)*4000 ); 
        end
    end
...
if (countCHs > 0)
    ...
    Sum_Fraction = Sum_Fraction + Number_Advanced_Nodes/countCHs;
    Fraction_CH (Effective_Number_Rounds, 2) = Number_Advanced_Nodes/countCHs;
else
%     CH_Energy (r, 2) = 0;
end
...
if (countCHs >0)
    for nodes = 1 : n
        Energy_Nodes (nodes, 1) = S(nodes).E;
    end    
...
voronoi (Cluster_Initial(1:countCHs, 1), Cluster_Initial(1:countCHs, 2),'d');
hold on;
pause;

plot (Cluster(1:countCHs, 1), Cluster(1:countCHs, 2), 's');
hold on;
pause;
