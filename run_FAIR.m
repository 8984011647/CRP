function [Number_Nodes_Alive, first_dead, Throughput, CH_Energy, Std_Energy, Average_Cluster_Head_Energy, Average_Standard_Deviation_Energy, f, Fraction_CH] = run_FAIR (m, a, Num_Nodes, Popt)
f=0;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                      %
...
if (countCHs > 0)
    CH_Energy (r, 2) = Cluster_Head_Energy / countCHs;
    Sum_Cluster_Head_Energy  = Sum_Cluster_Head_Energy + CH_Energy (r, 2);
...
for i=1:1:n
   if ( S(i).type=='N' && S(i).E>0 )
     if(cluster-1>=1)
       min_dis=inf;%sqrt( (S(i).xd-S(n+1).xd)^2 + (S(i).yd-S(n+1).yd)^2 );
       min_dis_cluster=1;
       for c=1:1:cluster-1
...
if (countCHs >0)
    ...
    Std_Energy (r, 2) = std (Energy_Nodes);
    Sum_Standard_Deviation_Energy = Sum_Standard_Deviation_Energy + Std_Energy (r, 2);
end
% countCHs;
rcountCHs=rcountCHs+countCHs;
