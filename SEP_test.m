% function [Number_Nodes_Alive, first_dead, Throughput, CH_Energy, Std_Energy, Average_Cluster_Head_Energy, Average_Standard_Deviation_Energy, Average_Fraction, Fraction_CH] = run_SEP (m, a,Num_Nodes, Popt)
m = 0.2;
a = 3;
Num_Nodes=100;
Popt =0.1;
...
for i=1:1:n
 ...
 if( ( S(i).ENERGY==0 && ( temp_rand <= ( pnrm / ( 1 - pnrm * mod(r,round(1/pnrm)) )) ) )  )
            ...
            C(cluster).xd=S(i).xd;
            C(cluster).yd=S(i).yd;
            Cluster (countCHs, 1) = S(i).xd;
            Cluster (countCHs, 2) = S(i).yd;
            if (To_Plot == 1)
%                 plot(S(i).xd,S(i).yd,'*');
                hold on;
            end
...
hold on;
voronoi (Cluster(1:countCHs, 1), Cluster(1:countCHs, 2), 's');
hold on;
